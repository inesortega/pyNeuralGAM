    ...
    for type in types:
        try:
            ...
            for i,var in enumerate(X.columns):
                ...
                fig, axs = plt.subplots(nrows=1, ncols=1, figsize=(25,20))
                ...
                data['x'] = X.iloc[:,i]
                data['y'] = f.iloc[:,i]
                sns.lineplot(data = data, x='x', y='y', color='royalblue', linewidth=4)
                
                axs.set_xlabel(f"$X_{i}$")
                axs.set_ylabel(f"$f(x_{i})$")
