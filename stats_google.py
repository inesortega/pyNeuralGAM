    ...
    for type in plot_types:
        try:
            ...
            for j in range(1, RANGE_SIZE+1):
                variables = pd.read_csv("./{0}/{1}/{2}/variables.csv".format(DATA_FOLDER, j, type), index_col=0).reset_index(drop=True)
                variables_training = pd.read_csv("./{0}/{1}/{2}/variables_training.csv".format(DATA_FOLDER, j, type), index_col=0).reset_index(drop=True)
                ...
                y_pred = pd.read_csv("./{0}/{1}/{2}/y_pred.csv".format(DATA_FOLDER, j, type), index_col=0).reset_index(drop=True)
                all_pred = pd.concat([all_pred, y_pred], axis=1)

            err_train.to_csv("./{0}/err_train_{1}.csv".format(type, DATA_FOLDER), index=False)
            err_pred.to_csv("./{0}/err_pred_{1}.csv".format(type, DATA_FOLDER), index=False)
            err_train_rmse.to_csv("./{0}/err_train_rmse_{1}.csv".format(type, DATA_FOLDER), index=False)
            err_pred_rmse.to_csv("./{0}/err_pred_rmse_{1}.csv".format(type, DATA_FOLDER), index=False)
            training_seconds.to_csv("./{0}/training_seconds_{1}.csv".format(type, DATA_FOLDER), index=False)
            ...
            avg_var = avg_expected_loss - avg_bias
            ...
            mean_values["bias"] = avg_bias
            mean_values["var"] = avg_var
