    ...
    for type in plot_types:
        try:
            ...
            for j in range(1, RANGE_SIZE+1):
                variables = pd.read_csv("./results/{0}/{1}/variables.csv".format(j, type), index_col=0).reset_index(drop=True)
                ...
                y_pred = pd.read_csv("./results/{0}/{1}/y_pred.csv".format(j, type), index_col=0).reset_index(drop=True)
            ...
            pd.DataFrame.from_dict(mean_values, orient="index").transpose().to_csv("./{0}/mean_metrics.csv".format(type), index=False)
