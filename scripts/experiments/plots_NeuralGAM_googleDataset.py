...
if __name__ == "__main__":
    ...
    def compute_mean_estimations(X_train):
        ...
        for j in range(1, RANGE_SIZE+1):
            fs_estimated_i = pd.read_csv("./results_google_dataset_NeuralGAM/{0}/{1}/fs_test_estimated.csv".format(j, type), index_col=0).reset_index(drop=True)
    ...
    for type in plot_types:
        try:
            ...
            learned_patch = mpatches.Patch(color='green', label='Mean estimation across 1000 iterations')
            ...
            plt.tight_layout()
            #plt.show(block=True)
            plt.savefig("./NeuralGAM_googleDataset" + type + ".png", dpi = 300, bbox_inches = "tight")
