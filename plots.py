...
if __name__ == "__main__":
    ...
    def compute_mean_estimations(X_train):
        ...
        for j in range(1, RANGE_SIZE+1):
            ...
            for i, f in enumerate(f_list):
                #pd.concat(f_list[i][j], fs_estimated_i[str(i)], axis=1)
                #f_list[i][j] = fs_estimated_i[str(i)]
                f_list[i] = pd.concat([f_list[i], fs_estimated_i[str(i)]], axis=1)

            
            q975 = q975.append(fs_estimated_i.quantile(0.975).transpose())
            q025 = q025.append(fs_estimated_i.quantile(0.025).transpose())
    ...
    for type in plot_types:
        try:
            ...
            fig.suptitle("Theoretical training data and learned functions (100 iterations) \n " + type, fontsize=10)
            ...
            for i, term in enumerate(X_train.columns):
                ...
                # draw quantiles
                """q = pd.DataFrame()
                q['x'] =  np.random.uniform(-3,3,q975.shape[0])
                q['q975'] = q975[str(i)]
                q['q025'] = q025[str(i)]
                sns.lineplot(data = q, x='x', y='q975', color='grey', linestyle='--', alpha=0.5, ax=axs[i])
                sns.lineplot(data = q, x='x', y='q025', color='grey', linestyle='--', alpha=0.5, ax=axs[i])"""
            ...
            plt.tight_layout()
            #plt.show(block=True)
            plt.savefig("./" + type + ".png", dpi = 100, bbox_inches = "tight")
